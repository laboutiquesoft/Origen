...
interface EditEpsModalProps {
  eps: Eps | null;
  isOpen: boolean;
  onClose: () => void;
}

export const EditEpsModal: React.FC<EditEpsModalProps> = ({ eps, isOpen, onClose }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (data: EpsFormData) => {
      ...
      return await axios.put(`${cataloguesApi.baseUrl}/eps/${eps?.id_eps}`, payload, {
    ...
    onError: (error: any) => {
      const msg = error.response?.data?.message || 'Error al actualizar la EPS';
      showToast({
        type: 'error',
        title: 'Error',
        message: msg,
  ...
  return (
    <>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all flex flex-col max-h-[90vh]">
                  ...
                  <ModalHeader title="Editar EPS" onClose={() => setShowCancelConfirm(true)} />

                  <div className="flex-1 overflow-y-auto p-6">
                    <form id="edit-eps-form" onSubmit={handleSubmit(onSubmitForm)} className="space-y-6">
                      <div>
                        <FormSectionHeader
                            ...
                            <Controller
                              ...
                              render={({ field: { value, onChange } }) => (
                                <Toggle
                                  ...
                                  description={value ? "La EPS se encuentra activa." : "La EPS se encuentra inactiva."}
                                  activeIcon={faCheck}
      ...
      <ConfirmationModal
        ...
        description="Si cancelas, los cambios no guardados se perderán."
      ...
      <ConfirmationModal
        ...
        description={`¿Deseas actualizar la EPS "${pendingFormData?.eps_name || ''}"?`}
